const run = () => {
    ...
    board = ['.'.repeat(board[0].length), ...board] //mock a useless 1st line

    let prevLine = board[0];
    let curLine;
    let futureLine;
    ...
    for(let i = 1; i < board.length; i++) {
        ...
        curLine = `${board[i]}.`
        if(i == (board.length - 1)) {
            futureLine = '.'.repeat(board[0].length) //mock a useless end line
        } else {
            futureLine = board[i+1]  
        }

        let curNum = ''
        let inSymbolRange = false;
        let symbolDiagonalLeading = false;
        let validSymbol = ''
        let symbLoc = ''

        //Move thru the current line. Track if symbol is currently in range.
        for(let j = 0; j < curLine.length; j++) {

            //Preface curline checks with top and bottom sliding windows.
            if(j > 0 && prevLine[j-1] != '.' && prevLine[j-1] != '\n' && !nums.includes(prevLine[j-1]) && (nums.includes(curLine[j]) || curNum.length !== 0)) {
                if(!inSymbolRange) {
                    inSymbolRange = true;
                    symbolDiagonalLeading = (j-1);
                    validSymbol = prevLine[j-1];
                    symbLoc = `${i-1}:${j-1}`;
                }
            }
            else if(j <= curLine.length && prevLine[j+1] != '.' && prevLine[j+1] != '\n' && !nums.includes(prevLine[j+1]) && (nums.includes(curLine[j]) || curNum.length !== 0)/* && !symbolDiagonalLeading*/) {
                if(!inSymbolRange) {
                    inSymbolRange = true;
                    symbolDiagonalLeading = (j+1);
                    validSymbol = prevLine[j+1];
            ...
            else if(j > 0 && futureLine[j-1] != '.' && futureLine[j-1] != '\n' && !nums.includes(futureLine[j-1]) && (nums.includes(curLine[j]) || curNum.length !== 0)) {
                if(!inSymbolRange) {
                    inSymbolRange = true;
                    symbolDiagonalLeading = (j-1);
                    validSymbol = futureLine[j-1];
                    symbLoc = `${i+1}:${j-1}`;
                }
            }
            else if(j <= curLine.length && futureLine[j+1] != '.' && futureLine[j+1] != '\n' && !nums.includes(futureLine[j+1]) && (nums.includes(curLine[j]) || curNum.length !== 0)/* && !symbolDiagonalLeading*/) {
                if(!inSymbolRange) {
                    inSymbolRange = true;
                    symbolDiagonalLeading = (j+1);
                    validSymbol = futureLine[j+1];
            ...
            else if(futureLine[j] !== '.' && futureLine[j] !== '\n' && !nums.includes(futureLine[j]) && (nums.includes(curLine[j]) || curNum.length !== 0)) {
                inSymbolRange = true;
                symbolDiagonalLeading = j;
                validSymbol = futureLine[j];
                symbLoc = `${i+1}:${j}`;
            }
            else if(prevLine[j] !== '.' && prevLine[j] !== '\n' && !nums.includes(prevLine[j]) && (nums.includes(curLine[j]) || curNum.length !== 0)) {
                inSymbolRange = true;
                symbolDiagonalLeading = j;
                validSymbol = prevLine[j];
                symbLoc = `${i-1}:${j}`;
            }


            if(Number.isInteger(parseInt(curLine[j]))) {
                curNum = `${curNum}${curLine[j]}`;
            } else if (curLine[j] !== '.') {
                inSymbolRange = true;
                validSymbol = curLine[j];
                symbLoc = `${i}:${j}`;

                //Did we just leave a number?
                if(curNum.length !== 0) {
                    if(validSymbol == '*') {
                        geartable.push(`${symbLoc}:${curNum}`)
                    }
                ...
                if(curNum.length !== 0) {
                    if(inSymbolRange) {
                        if(symbolDiagonalLeading && symbolDiagonalLeading > j) {
                            //invalid
                        } else if (j == (curLine.length - 1)) { //check that numbers at EOL have a valid identifier
                            if(validSymbol) {
                                if(validSymbol == '*') {
                                    geartable.push(`${symbLoc}:${curNum}`)
                                }
                ...
                //Wipe out symbol in range
                inSymbolRange = false;
                symbLoc = ''
                symbolDiagonalLeading = false;
            }
        }
    }
    console.log(geartable)
    let checktable = new Map()
    let sumTable = new Map()
    let gearTotal = 0;
    geartable.forEach(gear => {
        let position = gear.substring(0, gear.lastIndexOf(':'));
        let num = gear.substring(gear.lastIndexOf(':')+1);
        if(checktable.get(position) == undefined){
            checktable.set(position, 1);
            sumTable.set(position, parseInt(num))
        } else if (checktable.get(position) == 1) {
            checktable.set(position, 2);
            let temp = sumTable.get(position)
            sumTable.set(position, parseInt(temp) * parseInt(num))
        } else {
            console.log('found a 3-prong gear! ' + position)
        }
    });

    checktable.forEach((count, posi) => {
        if(count == 2) {
            gearTotal += parseInt(sumTable.get(posi));
        }
    }) 
...
const input = `48.................501....33.....622..............763.........331.................161.683......................................980..........
