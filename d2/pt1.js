const run = () => {
    let total = 0;
    const rmax = 12, gmax = 13, bmax = 14;
    input.split('\n').forEach(ln => {
        const line = ln.substring(ln.indexOf(':')+2)
        const gameNum = parseInt(ln.substring(ln.indexOf(' ')+1, ln.indexOf(':')))
        let gameData = {'red': 0, 'green': 0, 'blue': 0}
        line.split('; ').forEach(round => {
            round.split(', ').forEach(cubes => {
                let num = parseInt(cubes.substring(0, cubes.indexOf(' ')))
                let col = cubes.substring(cubes.indexOf(' ')+1)
                if(num > gameData[col]) {
                    gameData[col] = num;
                }
            })
        })
        if(gameData['red'] > rmax || gameData['green'] > gmax || gameData['blue'] > bmax) {
            //console.log(`game ${gameNum} is illegal`)
        } else {
            total += gameNum
        }
    });
...
const testin = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green`

const input = `Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
