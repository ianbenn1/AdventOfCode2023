const run = () => {
    ...
    input.split('\n').forEach(line => {
        let firstNum;
        let firstNumPosition = line.length+1;
        let lastNum;
        let lastNumPosition = 0;
        //Find first number
        for (let i = 0; i < line.length; i++) {
            if(Number.isInteger(parseInt(line[i]))) {
                firstNum = parseInt(line[i]);
                firstNumPosition = i;
                break;
            }
        }
        //Find first word
        outer: for (let i = 0; i < firstNumPosition; i++) {
            for (vn of validNums) {
                if(line.substring(i).indexOf(vn) == 0) {
                    firstNum = parseInt(validNums.indexOf(vn))+1;
                    break outer;
                }
            }
        }
        ...
        for (let i = line.length+1; i >= 0; i--) {
            if(Number.isInteger(parseInt(line[i]))) {
                lastNum = parseInt(line[i]);
                lastNumPosition = i;
                break;
            }
        }
        // //Find last word
        outer2: for (let i = line.length+1; i >= lastNumPosition; i--) {
            for (vn of validNums) {
                if(line.substring(i).indexOf(vn) == 0) {
                    lastNum = parseInt(validNums.indexOf(vn))+1;
                    break outer2;
                }
            }
        }
        total += parseInt(`${firstNum}${lastNum}`)
    });
    console.log(total)
}
...
const validStarts = [ 'o', 't', 'f', 's', 'e', 'n']

const testin = `two1nine
eightwothree
abcone2threexyz
xtwone3four
4nineeightseven2
